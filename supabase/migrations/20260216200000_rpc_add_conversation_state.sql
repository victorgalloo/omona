-- Add conversation_states to get_webhook_context RPC
-- Eliminates a separate query (~50-200ms) in processMessageGraph()
--
-- The existing function is replaced (CREATE OR REPLACE) with
-- a new version that also returns `conversationState` in the JSONB result.
...
BEGIN
  ...
  -- ============================================
  -- 11. CONVERSATION STATE (for LangGraph)
  -- ============================================
  SELECT row_to_json(cs.*)::jsonb INTO v_conversation_state
  FROM conversation_states cs
  WHERE cs.conversation_id = v_conv_id;
  ...
  RETURN jsonb_build_object(
    ...
    'conversationState', v_conversation_state
  );
END;
$$;
